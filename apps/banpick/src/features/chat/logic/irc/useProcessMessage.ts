...
export const useProcessMessage = () => {
    ...
    const processIncomingMessage = async (
        ...
        // 프로필 이미지 가져오기 (없을 때)
        const twitchAccessToken = accessTokens.twitch;
        if (incoming.provider === "twitch" && user.iconurl === "" && twitchAccessToken) {
            const cachedImageUrl = twitchProfileImageCache.current.get(incoming.id);
            if (cachedImageUrl !== undefined) {
                user.iconurl = cachedImageUrl;
            } else {
                try {
                    const userRes = (await apiGetUsers(incoming.id, twitchAccessToken)).data;
                    const imageUrl = userRes?.data?.[0]?.profile_image_url;
                    if (typeof imageUrl === "string") {
                        twitchProfileImageCache.current.set(incoming.id, imageUrl);
                        user.iconurl = imageUrl;
                    }
                } catch {
                    // 프로필 조회 실패가 채팅 메시지 처리까지 막으면 안 돼.
                }
            }
        }
