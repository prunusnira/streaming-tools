...
const chatApiBaseUrl = "https://openapi.chzzk.naver.com/open/v1/sessions";
...
export const chzzkProvider: OAuthProvider = {
    ...
    async createChatSession({ accessToken }) {
        const response = await fetch(`${chatApiBaseUrl}/auth`, {
            headers: { Authorization: `Bearer ${accessToken}` },
        });
        if (!response.ok) throw new Error("치지직 채팅 세션 생성에 실패했어.");

        const payload = (await response.json()) as { content?: { url?: string } };
